...
class XiaomiBluetoothDeviceData(BluetoothData):
    ...
    def _start_update(self, service_info: BluetoothServiceInfo) -> None:
        ...
        _LOGGER.warning("Parsing Xiaomi BLE advertisement data: %s", service_info)
...
#         )
        ...
        if frctrl_mac_include != 0:
            i += 6
            if msg_length < i:
                _LOGGER.warning("Invalid data length (in MAC check), adv: %s", data.hex())
                return False
...
#         )
        try:
            ...
            _LOGGER.warning("Unknown Xiaomi device found. Data: %s", data.hex())
            return False
        ...
        is_ptx = device_type in ["PTX_YK1_QMIMB"]
    ...
    def _decrypt_mibeacon_v4_v5(
        ...
        if len(data) < i + 9:
            _LOGGER.warning("Invalid data length (for decryption), adv: %s", data.hex())
            return None

        if not self.bindkey:
            self.bindkey_verified = False
            _LOGGER.warning("Encryption key not set and adv is encrypted")
            return None
        ...
        try:
            ...
            _LOGGER.warning("mic: %s", mic.hex())
            _LOGGER.warning("nonce: %s", nonce.hex())
            _LOGGER.warning("encrypted payload: %s", encrypted_payload.hex())
            return None
